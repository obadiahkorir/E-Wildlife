...
<?php
$update=$_GET['update'];
$result = mysqli_query($conn,"SELECT * FROM animals where animals_id ='$update'");
?>
<?php if($row = mysqli_fetch_array($result))
  {?> 
	...
	<input type="text"  value="Quick Search" onfocus="if(!this._haschanged){this.value=''};this._haschanged=true;">
		...
		<th colspan="3"><h2>UPDATE ANIMALS:</h2> </th> 			
	<input type="hidden" id="ID" name="ID" value="<?php echo $row['animals_id'];?>"  placeholder="ID" required>
	<tr>
    <td>  
		<label> Animal Name:</label>
		<input type="text" name="a_name" id="wname"  value="<?php echo $row['animal_name'];?>"  placeholder="Warehouse Name" required>
		<span class="error">This is an error</span>
	</td>
   <td>  
	<label>County:</label>
		<select name="country" id="select"  value="<?php echo $row['county'];?>"  placeholder="Select Country" required>
   ...
   <td>  
	...
	<input type="text" name="email" id="email" value="<?php echo $row['Gamepark'];?>"  placeholder="Email" required>
	<span class="error">This is an error</span>	
	</td>
   </tr>
  <tr>
...
<label> Description:</label>
	<input type="text" name="address"  id="address" value="<?php echo $row['Description'];?>"  placeholder="Address" required>
		<span class="error">This is an error</span>                         
	</td>
   <td>  
   <label>Entry Fee:</label>
	<input type="text" name="city"  id="city" value="<?php echo $row['entry_fee'];?>"  placeholder="City" required>
<span class="error">This is an error</span>		
  </td>
   <td>         
   <label>Tota Animals:</label>
	<input type="text" name="pcode"  id="pcode" value="<?php echo $row['total_animals'];?>"  placeholder="Postal Code" required>
...
<div class="bottom">
<td colspan="3">	
<button type="submit"  name="submit"  class="a-btn"><span class="a-btn-text">Update Animals</span></button>			
<div class="clear"></div>
</div>
