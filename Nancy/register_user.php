...
$con_email=mysqli_query($mysqli,"SELECT email FROM registration WHERE email='$email'");
$count=mysqli_num_rows($con_email);
if($count==0){
$sql=mysqli_query($mysqli,"INSERT INTO registration(username,email,category,password,date)VALUES('$username','$email','$category','$password','$date')");
        if(!$sql){
